...
function parseProvinceNews(rawText) {
  ...
  function recordTop(date, desc, impact) {
  ...
  for (const line of rawLines) {
    ...
    for (var si=0; si<SPELLS.length; si++) {
      var sp = SPELLS[si];
      var re = new RegExp(sp.pattern, 'i');
      if (re.test(ev)) {
        var name = sp.name;
    ...
    var thiefOpMatched = false;
    for (var oi=0; oi<THIEVERY_OPS.length; oi++) {
      var op = THIEVERY_OPS[oi];
      var reOp = new RegExp(op.pattern, 'i');
      if (reOp.test(ev)) {
        thiefOpMatched = true;
        ag.thief_incidents = (ag.thief_incidents||0) + 1;
        ag.thief_origins['Unknown Province'] = ag.thief_origins['Unknown Province'] || 0;
  ...
  const meteorTroops = (ag.meteor_kills.peasants||0) + (ag.meteor_kills.soldiers||0) + (ag.meteor_kills.quickblades||0) + (ag.meteor_kills.pikemen||0) + (ag.meteor_kills.golems||0) + Object.values(ag.meteor_kills.others||{}).reduce((s,v)=>s+v,0);

  // peasants killed in raids (massacres)
  let peasantsMassacred = 0;
  for (const r of ag.raids) peasantsMassacred += (r.civKilled||0);

  // Thievery breakdown
  const thieveryMap = {};
  let knownThievesTotal = 0;
  for (const k of Object.keys(ag.thief_origins||{})) { thieveryMap[k]=ag.thief_origins[k]; knownThievesTotal += ag.thief_origins[k]; }
  ...
  const raidsList = (ag.raids||[]).slice().sort((a,b)=>b.civKilled - a.civKilled);
  ...
  if (stolenRunesLine) {
    ...
    if (m) { lines.push(''); lines.push('Stolen Resources:'); lines.push(`Runes: ${m[1]}`); }
  }
  lines.push('');
  ...
  for (const r of raidsList) {
    ...
    for (const k of Object.keys(ul)) {
      const key = k.toLowerCase();
      const v = ul[k];
      if (key.includes('pik')) pik += v;
      else if (key.includes('golem')) gole += v;
      else if (key.includes('quick')) quick += v;
    }
    const troopParts = [];
...
module.exports = { parseProvinceNews, default: parseProvinceNews };
